...
obscureCounts <- function(table,
  ...
  if (!is.null(minCellCount)) {
    ...
    checkColNames <- NULL
    if (grepl("drugRoutes|drugSig|drugVerbatimEndDate|drugQuantity|drugSourceConcepts|drugTypes|drugExposureDuration|missingValues|drugDose|drugDaysSupply", tableName)) {
   ...
   } else if (grepl("drugIngredientOverview|drugIngredientPresence", tableName)) {
      checkColNames <- colNames[grepl("n_people", colNames)]
    }
